...
with taberp:
    ...
    with st.expander('Cluster Estado:'):
        clu_est_cus = clu_est[
            clu_est['reference']== base_con['State'].values[0]
        ]
...
with tabbi:
    ...
    with st.expander('Região'):
        ...
        regiao = st.selectbox(
            'Região:', 
            rg_reg['Region'].unique()
        )
        
        gr_reg = rg_reg[
            rg_reg['Region'] == regiao
        ].groupby('ano')['yhat'].sum().reset_index()
        ...
        gr_base = base[
            base['Region'] == regiao
        ].groupby('OYear')['Sales'].sum().reset_index()
        
        real = alt.Chart(gr_base).mark_line(
            color='red'
        ).encode(x='OYear', y='Sales')
    ...
    with st.expander('Segmento'):
        ...
        if (st.checkbox('Entregas do Segmento (Mapa):')):
            ...
            coords_vendas_seg = base_seg.groupby(
                'State'
            )['Sales'].sum().reset_index()
            
            coords_vendas_seg = coords_vendas_seg.merge(
                estados, 
                left_on=['State'], 
                right_on=['name'], 
                how='left'
